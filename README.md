...
|---|---|---|---|
| From scratch (`yolov8n.yaml`) | 0.00 | 0.00 | No |
| Transfer learning (`yolov8n.pt`) | **0.729** | **0.553** | Yes — by epoch 5 |

Training hardware: Tesla T4, 50 epochs, batch 16, 640x640, seed 0.
Full metrics and per-epoch learning curves are in [results/metrics.md](results/metrics.md).
...
and writes the annotated outputs to `results/figures/` on your local machine.
...
To use a larger GPU (e.g. A10G):

```python
# src/modal_train.py, line 14
GPU = "A10G"
```
...
The from-scratch model produced no detections at any confidence threshold
across all 50 training epochs. The transfer-learning model reached a usable
mAP@0.5 of 0.615 at epoch 1 — before the from-scratch model had formed a
single stable feature map.
...
The transfer-learning model reached mAP@0.5 > 0.70 by epoch 9, demonstrating
fast convergence characteristic of representation reuse. The loss landscape
seen by the optimizer starts near a well-conditioned local minimum rather than
